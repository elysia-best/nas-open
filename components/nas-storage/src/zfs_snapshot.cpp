...
std::expected<void, std::string> ZfsSnapshotManager::create_snapshot(const std::string& dataset, const std::string& snap_name) {
    auto res = detail::RunCommand("zfs snapshot " + dataset + "@" + snap_name);
    if (!res.has_value()) return std::unexpected(res.error().message);
    return {};
}

std::expected<void, std::string> ZfsSnapshotManager::send_snapshot(const std::string& snapshot, const std::string& target, bool incremental) {
